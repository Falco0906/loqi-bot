...
from services.job_engine.models import BatchItem, Job


async def test_enqueue_persists_items_before_queueing(monkeypatch):
    captured = {}

    async def create_batch_job(job, items):
        captured["job"] = job
...
async def test_draft_batch_runner_completes_durable_item(monkeypatch):
    ...
    class Storage:
        ...
        def mark_batch_item_completed(self, *_): updates.append("completed"); return True
    ...
    monkeypatch.setattr("services.workspace_state.persist_draft_awaited", lambda *_: asyncio.sleep(0, result=True))
    ...
    result = await drafts.run_draft_batch_job(
        Job(id="job", user_id="user", type="draft_batch", workspace_id="workspace", campaign_id="campaign", payload={}),
        lambda *_: None,
    )
