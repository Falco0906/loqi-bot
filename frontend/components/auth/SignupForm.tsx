...
type Props = {
  onEmailSubmitted: (sessionId: string) => void;
};

export default function SignupForm({ onEmailSubmitted }: Props) {
  const { signup, storePendingRegistration, initiateGoogleLogin } = useAuth();
  ...
  const validate = (): boolean => {
    ...
    // Password Validation (must match backend PasswordConfig)
    if (!password) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const sessionId = await signup(email.trim());
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5" noValidate>
      <div>
        ...
        {fieldErrors.displayName && (
          <p className="mt-1 font-['Inter'] text-[13px] text-[#dc2626]">{fieldErrors.displayName}</p>
        )}
      </div>
      ...
      <div>
        ...
        />
        {fieldErrors.email && (
          <p className="mt-1 font-['Inter'] text-[13px] text-[#dc2626]">{fieldErrors.email}</p>
        )}
      </div>
      ...
      <div>
        ...
        {fieldErrors.organizationName && (
          <p className="mt-1 font-['Inter'] text-[13px] text-[#dc2626]">{fieldErrors.organizationName}</p>
        )}
      </div>
      ...
      <div>
        ...
        <input
          ...
          className="w-full bg-[#ffffff] border border-[#c4c7c7] rounded-lg px-4 py-3 font-['Inter'] text-[16px] leading-[1.5] text-[#1c1b1b] placeholder:text-[#747878] focus:outline-none focus:ring-1 focus:ring-[#000000] focus:border-[#000000] transition-all disabled:opacity-50"
        />
        <p className="mt-1 font-['Inter'] text-[12px] text-[#747878]">
          Must be at least 12 characters with uppercase, lowercase, number, and special character
        </p>
        {fieldErrors.password && (
          <p className="mt-1 font-['Inter'] text-[13px] text-[#dc2626]">{fieldErrors.password}</p>
        )}
      </div>
