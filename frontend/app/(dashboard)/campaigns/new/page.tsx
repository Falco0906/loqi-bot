...
import { generateCampaignStrategy, saveCampaign } from "../../../../lib/api";
...
export default function NewCampaignPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const strategy = await generateCampaignStrategy(token, id);
      if (!strategy.ok) throw new Error("Campaign was created, but strategy generation failed");
      toast("success", "Campaign strategy is ready for review");
    ...
    <WorkspaceContainer>
      <AppPage>
        <div className="reading-column py-16">
          ...
          <p className="text-base text-on-surface-variant/60 mb-10">Name the campaign and describe what this outreach should achieve. Loqi will prepare a strategy for your approval.</p>
          ...
          <form onSubmit={submit} className="space-y-6 max-w-2xl">
            ...
            <div className="flex gap-3 pt-3">
              ...
              <button type="submit" disabled={saving} className="rounded-lg bg-primary px-6 py-2.5 text-sm font-semibold text-on-primary disabled:opacity-50">{saving ? "Preparing strategy…" : "Create & Generate Strategy"}</button>
            </div>
          </form>
        </div>
      </AppPage>
