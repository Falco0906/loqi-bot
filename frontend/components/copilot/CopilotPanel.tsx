...
export default function CopilotPanel({ width = 380, variant = "sidebar" }: { width?: number; variant?: "sidebar" | "page" }) {
  ...
  const history = variant === "page" ? (
    <aside className="w-60 shrink-0 border-r border-outline-variant/10 bg-surface-container-low/30 flex flex-col">
      <div className="flex items-center justify-between px-3 py-3 border-b border-outline-variant/10"><span className="text-label-sm uppercase tracking-wider text-on-surface-variant/60 font-semibold">Chats</span><button type="button" onClick={newChat} className="p-1 rounded-md text-on-surface-variant/50 hover:text-on-surface hover:bg-surface-high/60" aria-label="New chat"><Icon name="edit_square" className="text-sm" /></button></div>
    ...
    <div className={variant === "page" ? "h-full w-full flex" : "shrink-0 h-full overflow-hidden flex justify-end bg-surface-lowest transition-[width] duration-200 ease-out"} style={variant === "page" ? undefined : { width: open ? width : 0 }} role="dialog" aria-label="Loqi OS">
      <div className={`${variant === "page" ? "w-full" : "w-[380px] max-w-[92vw] border-l border-outline-variant/15 shadow-glass"} h-full flex overflow-hidden`} style={variant === "page" ? undefined : { width }}>
        ...
        <div className="flex-1 min-w-0 h-full flex flex-col overflow-hidden">
          <header className="flex items-center justify-between px-4 py-3 border-b border-outline-variant/10 shrink-0"><div className="flex items-center gap-2.5"><div className="w-7 h-7 rounded-lg bg-primary/15 flex items-center justify-center"><Icon name="smart_toy" className="text-[16px] text-primary" /></div><span className="text-body-md text-on-surface font-bold">Loqi OS</span></div><div className="flex items-center gap-2"><span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] uppercase tracking-wider font-semibold ${working ? "bg-primary/10 text-primary" : conversationState === "failed" ? "bg-error/10 text-error" : conversationState === "completed" ? "bg-success/10 text-success" : "bg-surface-high/60 text-on-surface-variant/60"}`}><span className={`w-1.5 h-1.5 rounded-full ${working ? "bg-primary animate-pulse" : "bg-current"}`} />{STATE_LABELS[conversationState]}</span><button type="button" onClick={clear} className="p-1.5 rounded-lg text-on-surface-variant/50 hover:text-on-surface hover:bg-surface-high/60" title="Clear conversation" aria-label="Clear conversation"><Icon name="delete_sweep" className="text-[18px]" /></button></div></header>
          ...
          <div className="shrink-0">{messages.length === 0 && <QuickReplies options={idleOptions} onSelect={handleQuickReply} />}{conversationState === "clarification" && <QuickReplies options={CLARIFICATION_REPLIES} onSelect={handleClarificationReply} />}<CopilotComposer onSend={startTask} disabled={working} placeholder={working ? "Loqi is working…" : "Message Loqi…"} /></div>
        </div>
      </div>
    </div>
  );
